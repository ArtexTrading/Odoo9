...
class HrAttendanceImport(models.TransientModel):
    ...
    def _movements_from_csv(self, reader):
        ...
        for index, row in enumerate(reader):
            ...
            # Init and load CSV row data
            employee_code, type_movement, date_movement = row
            values = {}
            values['incidence'] = ''

            # Verify Datetime data
            try:
                date_movement = fields.Datetime.from_string(date_movement)
            except:
                values['incidence'] += _('Datetime format incorrect!\n')
            ...
            if values['incidence']:
                ...
                if type_movement == 'Input':
                    values['check_in'] = date_movement
                elif type_movement == 'Output':
                    values['check_out'] = date_movement
        ...
        if len(attendance_lst) > 0:
            ...
            def _attendance_sort(_movement):
                return str(_movement['employee_id'])\
                    + _movement['date_movement'].strftime("%Y-%m-%d %H:%M:%S")\
            ...
            for movement in attendance_lst:
                if movement['employee_id'] != employee_id:
                    ...
                    if type_movement == 'Output':
                        attendance = attendance_obj.search([
                            ('employee_id', '=', employee_id),
                            ('check_in', '<=', date_movement.strftime("%Y-%m-%d %H:%M:%S")),
                            ('check_out', '=', False),
                ...
                if type_movement == movement['type_movement']:
                    if movement['type_movement'] == 'Input':
                        incidence_obj.create(
                            {'employee_id': movement['employee_id'],
                             'check_in': movement['date_movement'],
